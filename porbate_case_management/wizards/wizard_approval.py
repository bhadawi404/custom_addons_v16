...
class ReasonRejectHRO(models.TransientModel):
    ...
    def action_reject(self):
        ...
        for rec in case:
            ...
            template_approval = self.env.ref('porbate_case_management.email_reject', raise_if_not_found=False)
            template_context = {
                'administration': rec.administration_name.name,
                'message': self.remarks,
                'user_id': self.env.user.name,
                'company_id': self.env.company.name,
                'action_url': action_url,
            }
            template_approval.with_context(**template_context).send_mail(rec.id, force_send=True, email_values={'email_to': rec.administration_name.email}, email_layout_xmlid='mail.mail_notification_light')